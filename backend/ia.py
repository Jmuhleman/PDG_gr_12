    ...
    while True:
        ...
        for license_plate in license_plates:
            ...
            if license_plate_text:
                ...
                print(f"IN {frame_nmr}: Detected license plate {license_plate_text[0]} with confidence {score:.2f}")

                current_time = datetime.now(timezone.utc).replace(microsecond=0)
                timestamp = current_time.isoformat()
                timestamp_img = timestamp.replace(':', '-')
                
                output_filename = os.path.join(output_folder, f'{license_plate_text[0]}_in_{timestamp_img}.png')
                ...
                try:
                    ...
                    cursor.execute(query_plate_logs, (
    ...
    while True:
        ...
        for license_plate in license_plates:
            ...
            if license_plate_text:
                ...
                label = f"{license_plate_text[0]} ({score:.2f})"
                cv2.putText(frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 1.75, (0, 255, 0), 2)
                print(f"OUT {frame_nmr}: Detected license plate {license_plate_text[0]} with confidence {score:.2f}")

                current_time = datetime.now(timezone.utc).replace(microsecond=0) + timedelta(days=random.choice([0,1,2]), hours=random.randint(0,23), minutes=random.randint(0,59))

                timestamp = current_time.isoformat()
                timestamp_img = timestamp.replace(':', '-')

                output_filename = os.path.join(output_folder, f'{license_plate_text[0]}_out_{timestamp_img}.png')
                cv2.imwrite(output_filename, license_plate_crop)
                ...
                try:
                    ...
                    cursor.execute(query_update_plate, (timestamp, license_plate_text[0]))
                    conn.commit()
